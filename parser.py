...
import clang.cindex
from clang.cindex import Config, Index, TranslationUnit, CursorKind
...
def extract_defs(header_path):
    index = Index.create()
    tu = index.parse(header_path, options=TranslationUnit.PARSE_DETAILED_PROCESSING_RECORD)
    ...
    for node in tu.cursor.get_children():
        ...
        if node_file is not None and path.basename(node_file.name) != header_path:
            continue
    ...
    try:
        if args.clang_path:
            if path.isfile(args.clang_path):
                Config.set_library_file(args.clang_path)
            else:
